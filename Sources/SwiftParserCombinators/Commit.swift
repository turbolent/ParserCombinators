...
extension Parser {
    ...
    ///
    /// - Parameter next: A parser that will be applied on the remaining input after this parser succeeds.
    ///
    /// - Returns: A parser that -- on success -- returns a tuple that contains the result of this parser
    ///            and that of `next`. The parser fails if either this parser or `next` fail, and
    ///            the failure is fatal, i.e., an error.
    ///
...
/// Creates a new parser from the given parser, so that its failures become errors (the `||` combinator
/// will give up as soon as it encounters an error, on failure it simply tries the next alternative).
///
